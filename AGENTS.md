...
## Overview
Chrome Manifest V3 extension that lists all open (non-pinned) tabs in a popup, grouped by domain, with one-click close for individual tabs or entire domains.
...
- Pinned tabs are filtered out on load (intentionally preserved by the user).
...
- `count` — by number of tabs in the domain (desc), tie-broken alphabetically.
...
### Collapse State
- `collapsedDomains` is an in-memory `Set` of domain names that are collapsed.
...
- Domain counts in the badge reflect filtered tabs, not all tabs.
...
- The ✕ button on a domain header calls `chrome.tabs.remove([ids...])` for all tabs in that domain.
